...
# (ts, 包厢, 设备, 现象, 严重度, 描述, 店员) —— 调整条数时这里是唯一事实来源
DEMO_EVENTS = [
    # —— 簇A：K302 啸叫，跨话筒/功放，同包厢声学问题 ——
    ("2026-09-11T20:00", "K302", "MIC-302", "howl", "warn", "话筒一靠近音响就啸叫", "小王"),
    ("2026-09-11T20:08", "K302", "AMP-302", "howl", "warn", "话筒啸叫，功放峰值灯爆红", "小张"),
    ("2026-09-11T20:20", "K302", "MIC-302", "howl", "warn", "高音段话筒啸叫明显", "小王"),

    # —— 簇B：305/306/307 点歌延迟，汇聚到点歌服务器 ——
    ("2026-09-11T19:40", "K305", "POD-305", "song_delay", "warn", "点歌后转圈十几秒才开始播放", "小李"),
    ("2026-09-11T19:44", "K306", "POD-306", "song_delay", "warn", "切歌反应慢，歌单加载卡顿", "小赵"),
    ("2026-09-11T19:47", "K307", "POD-307", "song_delay", "warn", "点歌延迟严重，搜歌要转半天", "小李"),

    # —— 簇C：K301 电视单台偶发 ——
    ("2026-09-11T19:05", "K301", "TV-301", "video_break", "warn", "电视画面中断，重新开机后恢复", "小钱"),

    # —— 簇D：K304 点歌屏（误合并素材：显示故障 vs 触摸故障，措辞相近被 R2 合并） ——
    ("2026-09-11T20:10", "K304", "POD-304", "stuck", "warn", "点歌屏黑屏，重启后恢复正常", "小孙"),
    ("2026-09-11T20:18", "K304", "POD-304", "stuck", "warn", "点歌屏黑屏，触摸也没反应", "小孙"),

    # —— 独立事件：K304 功放杂音（同包厢但症状不同，不应被并入口簇D） ——
    ("2026-09-11T20:42", "K304", "AMP-304", "mic_hum", "info", "功放电流杂音，音量越大越明显", "小孙"),

    # —— 簇E：跨区画面中断（502 在5FC、303 在3FA，LCA=核心，疑似媒体服务器） ——
    ("2026-09-11T21:30", "K502", "TV-502", "video_break", "warn", "电视画面中断黑屏，声音正常", "小周"),
    ("2026-09-11T21:33", "K303", "TV-303", "video_break", "critical", "画面中断，换HDMI线也没用", "小王"),

    # —— 簇F：5FC 三个包厢离线，疑似区域交换机 ——
    ("2026-09-11T21:50", "K501", "POD-501", "offline", "critical", "点歌屏离线，重启连不上服务器", "小吴"),
    ("2026-09-11T21:52", "K502", "POD-502", "offline", "critical", "包厢点歌设备全部离线", "小吴"),
    ("2026-09-11T21:55", "K503", "POD-503", "offline", "critical", "设备离线，扫码点歌扫不出来", "小郑"),

    # —— 簇G：TV-308 单台间歇性复发（30分钟内3次，单台flapping） ——
    ("2026-09-11T18:02", "K308", "TV-308", "video_break", "warn", "电视画面中断几秒后自行恢复", "小冯"),
    ("2026-09-11T18:35", "K308", "TV-308", "video_break", "warn", "电视又断了一下，自己恢复了", "小冯"),
    ("2026-09-11T19:12", "K308", "TV-308", "video_break", "warn", "画面中断复发，怀疑线材接触不良", "小冯"),
]
DEMO_EVENT_COUNT = len(DEMO_EVENTS)


def seed_demo_events(create_event):
    """create_event: correlation.create_event(conn, payload) -> event row"""
    results = []
    for ts, room, dev, sym, sev, desc, reporter in sorted(DEMO_EVENTS, key=lambda x: x[0]):
